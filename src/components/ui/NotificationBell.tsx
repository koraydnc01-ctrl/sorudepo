...
export function NotificationBell() {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 sm:left-0 mt-2 w-72 max-w-[90vw] max-h-96 overflow-y-auto bg-white border border-line rounded-lg shadow-lg z-50">
          {!loading && items.length > 0 && (
            <div className="flex items-center justify-end px-3 py-2 border-b border-line">
              <button
                onClick={clearAll}
                disabled={clearing}
                className="text-xs text-muted hover:text-ink font-medium disabled:opacity-50"
                type="button"
              >
          ...
          {!loading &&
            items.map((n) => (
              <div
                key={n.id}
                className="flex items-start justify-between gap-2 px-3 py-2.5 border-b border-line last:border-b-0 text-sm"
              >
                <div>
                ...
                <button
                  ...
                  className="text-muted hover:text-ink text-base leading-none shrink-0 mt-0.5"
                  type="button"
