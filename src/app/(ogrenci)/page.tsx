...
// Fotoğrafı yüklemeden önce tarayıcıda sıkıştırır: en uzun kenarı maxDimension'a
// indirir, ardından JPEG kalitesini hedef boyuta (targetSizeKB) inene kadar
// kademeli olarak düşürür. Öğrenci deneyiminde görünür bir değişiklik olmaz,
// sadece seçimden hemen sonra birkaç yüz milisaniyelik bir işlem eklenir.
      ...
      const tryQuality = (quality: number) => {
        canvas.toBlob(
          (blob) => {
            ...
            // Hedefin üzerindeyse ve kaliteyi daha fazla düşürecek yer varsa tekrar dene.
            if (sizeKB > targetSizeKB && quality > 0.4) {
              tryQuality(Number((quality - 0.1).toFixed(2)));
              return;
            }
...
export default function SoruEklePage() {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Yükleme öncesi istemci tarafında sıkıştır; bir sebeple başarısız olursa
    // orijinal dosyayla devam et, akış bozulmasın.
    setCompressing(true);
    let toUse = selected;
    try {
      toUse = await compressImage(selected);
    } catch {
      toUse = selected;
    } finally {
    ...
    setFile(toUse);
    setPreviewUrl(URL.createObjectURL(toUse));

    // fotoğraf hazır olur olmaz AI konu tahmini iste (sıkıştırılmış görselle)
    setAiLoading(true);
    try {
      const base64 = await fileToBase64(toUse);
      const res = await fetch("/api/konu-tespit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          imageBase64: base64,
          mediaType: toUse.type || "image/jpeg",
        }),
      });
